...
if PANDAS_AVAILABLE:
    ...
    polarpandas_time = time_operation(
        lambda: ppd.DataFrame(data), "polarpandas DataFrame", 5
    )
...
if PANDAS_AVAILABLE:
    ...
    ppd_df = ppd.DataFrame(
        {"a": list(range(LARGE)), "b": list(range(LARGE, 2 * LARGE))}
    )
    ...
    polarpandas_time = time_operation(
        lambda: (lambda df: (df.__setitem__("c", df["a"] + df["b"]), df)[1])(
            ppd_df.copy()
        ),
        "polarpandas add column",
        10,
    )
    print(f"  polarpandas: {polarpandas_time * 1000:.2f} ms")
...
if PANDAS_AVAILABLE:
    ...
    polarpandas_time = time_operation(
        lambda: ppd_df.filter(ppd_df["a"] > 50000), "polarpandas filter", 10
    )
...
if PANDAS_AVAILABLE:
    ...
    pandas_time = time_operation(
        lambda: pd_df.groupby("group")["value"].mean(), "pandas groupby", 5
    )
    polarpandas_time = time_operation(
...
if PANDAS_AVAILABLE:
    ...
    polarpandas_time = time_operation(
        lambda: ppd_df.sort_values("a"), "polarpandas sort", 5
    )
...
if PANDAS_AVAILABLE:
    ...
    pd_df2 = pd.DataFrame(
        {"key": list(range(MEDIUM)), "value2": list(range(MEDIUM, 2 * MEDIUM))}
    )
    ...
    pandas_time = time_operation(
        lambda: pd_df1.merge(pd_df2, on="key"), "pandas merge", 5
    )
    polarpandas_time = time_operation(
        lambda: ppd_df1.merge(ppd_df2, on="key"), "polarpandas merge", 5
    )
...
if PANDAS_AVAILABLE and results:
    ...
    for operation, pd_time, ppd_time in results:
        ...
        print(
            f"{operation:<40} {pd_time * 1000:>10.2f}ms {ppd_time * 1000:>10.2f}ms {speedup:>15}"
        )
